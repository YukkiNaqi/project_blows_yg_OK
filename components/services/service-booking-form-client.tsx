...
interface ServiceBookingFormClientProps {
  service: ServiceType;
}

export function ServiceBookingFormClient({ service }: ServiceBookingFormClientProps) {
  ...
  return (
    <>
      <Button onClick={openBookingForm} className="w-full group-hover:bg-primary/90 transition-colors">
        Pesan Layanan
      </Button>
      {showBookingForm && (
        <ServiceBookingModal 
...
function ServiceBookingModal({ service, onClose }: ServiceBookingModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Call the API to create a service booking
      const response = await fetch('/api/admin/services', {
        ...
        body: JSON.stringify({
          ...
          service_type: service.name, // Use the service name as the type
      ...
      if (response.ok) {
        ...
        setError(errorData.error || 'Gagal membuat booking. Silakan coba lagi.');
      }
    } catch (error) {
